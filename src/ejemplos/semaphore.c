//If using a mutex and two semaphores to protect some data being passed between threads seems overly complicated, 
//you're right! Semaphores have their uses, but you'll often find that you can pass data more easily using a queue.
//As a result, here's a much better way to accomplish the above challenge using just one queue.
